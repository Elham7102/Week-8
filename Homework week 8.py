# -*- coding: utf-8 -*-
"""Copy of python_week1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1FFhhH8endDxQrHx_4V04UXzA0gw9ufww
...
for participant_id in range(1, 21):
  for x in ["high","low"]:
    health_block = df[(df['participant'] == participant_id) & (df['block'] == 'health') & (df['fat'] == x)]
    valid_ratings = health_block['rating'].dropna()
    average_health_rating = valid_ratings.mean()
    print(f"Participant {participant_id}'s average health rating for {x} fat: {average_health_rating:.2f}")
...
for participant in range(1,3):
  for x in [1,0]:
    ...
    print(f"Participant {participant}'s average RT rating: {average_RT:.2f}")
...
for participant in participants:
    for delayed in [1, 0]:
        ...
        for i in range(len(data)):
            if data.loc[i, 'participant'] == participant and data.loc[i, 'delayed_opt_chose'] == delayed:

                if data.loc[i, 'choice'] == 1.0:
                    earnings += data.loc[i, 'money_left']
                elif data.loc[i, 'choice'] == 2.0:
                    earnings += data.loc[i, 'money_right']


        total_earnings[(participant, delayed)] = earnings
        print(f"Participant {participant} (delayed={delayed}) total earnings: {earnings:.2f}")
